...
public:
	...
	**/
	...
	COMPLEJIDAD: Determinar aquí, justificadamente, la complejidad de esta operación
		La compeljida de la operacion es lineal porque como mucho va a recorrer todos los nodos de la cola en el bucle While.	
		El resto de operaciones son de coste constante (asiganaciones, resta).
		
	**/

	void llevarAlPrincipio(unsigned int pos) {
		if (esVacia()|| pos > longitud() || pos <= 0){
			throw EPosicionInvalida();
		}
		if (pos > 1) {
			unsigned int cont = 1;
			Nodo *recorriendo = _prim;
			Nodo *cambio = nullptr;

			while (cont < pos) {
				if (cont != pos - 1)
					recorriendo = recorriendo->_sig;
				else
					cambio = recorriendo->_sig;
				cont++;
			}

			recorriendo->_sig = cambio->_sig;
			cambio->_sig = _prim;
			_prim = cambio;
			...
			recorriendo = nullptr; delete recorriendo;
			cambio = nullptr; delete cambio;
		}
	}
